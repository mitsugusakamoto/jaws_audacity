...
VSTプラグインでは:
プリセットコントロールにフォーカスを移動するには%keyfor (VSTPreset)を押します。
すでにあるプリセットを読み込むには%keyfor (VSTLoadPreset)を押します。
現在の設定をプリセットとして保存するには%keyfor (VSTSavePreset)を押します。
...
設定ダイアログを表示するにはCtrl + P を押します。
ツールバー、トラックパネル、選択バーを順方向に移動するにはCtrl + F6 を押します。
ツールバー、トラックパネル、選択バーを逆方向に移動するにはCtrl + Shift + F6 を押します。
波形を元の縮尺に戻すにはCtrl + 2 を押します。
波形を拡大するにはCtrl + 1 を押します。
波形を縮小するにはCtrl + 3を押します。

再生関連の相さ
コマンドとキーの割り当て 
再生/停止をするには スペースバーを押します。
カーソルを移動させながらの再生/停止をするには Shift + Aを押します。
一時停止/再開をするにはPを押します。
